Algoritmo ejercicio2
	...
	Mientras (opc=='S' O opc=='s') Hacer
		...
		// Comprobamos que la duración total de las canciones no sea mayor a 74 minutos
		Si (dTotal>=(74*60)) Entonces
			opc <- 'N'
		SiNo
			Escribir '¿Desea ingresar otra canción? (S/N)'
			Leer opc
	...
	dTotal <- dTotal MOD 3600
	dMin <- trunc(dTotal/60) // horas en el total de segundos
	dSeg <- dTotal MOD 60 // resto
	// Mostramos la duración total de las canciones
	Escribir 'Duración Total de las canciones: ', dHora, ':', dMin, ':', dSeg // minutos en el total de segundos
FinAlgoritmo // resto
